...
if task == 'ae':
    ...
    # reconstructed:(10000,32,32,3)
    reconstructed=np.concatenate(reconstructed, axis=0)

    # anomality其實就是mean square error
    anomality=np.mean(np.square(reconstructed - y).reshape(len(y), -1), axis=1)
    y_pred=anomality
    MSE_index=anomality.argsort()

    # df = pd.DataFrame({"MSE Error":anomality})
    # df.describe().to_csv('./result/VAE/MSE_describe.csv')
    # fig = plt.figure()
    # ax = fig.add_subplot(111)
    # _ = ax.hist(anomality, bins=10)
    # plt.savefig("./result/VAE/MSE_distribution.png")
    # plt.close()

    reconstructed= reconstructed.reshape(len(reconstructed),32,32,3)
    reconstructed , test = normalize(reconstructed, test)
    # Draw the image with largest MSE
    # print("The {}-th image has the largest MSE {:^.6f}".format(
    #     MSE_index[-1], anomality[MSE_index[-1]]))
    # # print("Draw the Image {}".format(MSE_index[-1]))
    # plt.figure()
    # plt.title("Image with largest MSE {:^.6f}".format(
    #     anomality[MSE_index[-1]]))
    # plt.subplot(1, 2, 1)
    # plt.imshow(np.array(test[MSE_index[-1]]))
    # plt.xlabel("Original Image")
    # plt.subplot(1, 2, 2)
    # plt.imshow(np.array(reconstructed[MSE_index[-1]]))
    # plt.xlabel("Reconstructed Image")
    # plt.savefig('./result/VAE/largest_MSE.png')
    # plt.close()

    # # Draw the image with 2-nd largest MSE
    # print("The {}-th image has the 2-nd largest MSE {:^.6f}".format(
    #     MSE_index[-2], anomality[MSE_index[-2]]))
    # plt.figure()
    # plt.title("Image with 2-nd largest MSE {:^.6f}".format(
    #     anomality[MSE_index[-2]]))
    # plt.subplot(1, 2, 1)
    # plt.imshow(np.array(test[MSE_index[-2]]))
    # plt.xlabel("Original Image")
    # plt.subplot(1, 2, 2)
    # plt.imshow(np.array(reconstructed[MSE_index[-2]]))
    # plt.xlabel("Reconstructed Image")
    # plt.savefig('./result/VAE/2-nd_largest_MSE.png')
    # plt.close()

    # # Draw the image with smallest MSE
    # print("The {}-th image has the smallest MSE {:^.6f}".format(
    #     MSE_index[0], anomality[MSE_index[0]]))
    # plt.figure()
    # plt.title("Image with smallest MSE {:^.6f}".format(
    #     anomality[MSE_index[0]]))
    # plt.subplot(1, 2, 1)
    # plt.imshow(np.array(test[MSE_index[0]]))
    # plt.xlabel("Original Image")
    # plt.subplot(1, 2, 2)
    # plt.imshow(np.array(reconstructed[MSE_index[0]]))
    # plt.xlabel("Reconstructed Image")
    # plt.savefig('./result/VAE/smallest_MSE.png')
    # plt.close()
    # # Draw the image with 2-nd smallest MSE
    # print("The {}-th image has the 2-nd smallest MSE {:^.6f}".format(
    #     MSE_index[1], anomality[MSE_index[1]]))
    # plt.figure()
    # plt.title("Image with 2-nd smallest MSE {:^.6f}".format(
    #     anomality[MSE_index[1]]))
    # plt.subplot(1, 2, 1)
    # plt.imshow(np.array(test[MSE_index[1]]))
    # plt.xlabel("Original Image")
    ...
    # plt.imshow(np.array(reconstructed[MSE_index[1]]))
    # plt.xlabel("Reconstructed Image")
    # plt.savefig('./result/VAE/2-nd_smallest_MSE.png')
    # plt.close()
